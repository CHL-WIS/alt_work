function ww3_alt(alt_data,loc,coord,res)
...
%     end
% end
for qq = 1:1%size(alt_data,2)
[sat.mod_bias,sat.mod_rmse,sat.alt_mean,sat.mod_si] = alt_stat2(mod_bias{qq},mod_rmse{qq}, ...
   alt_out{qq});
% for jj = 1:size(alt_mean{1},2)
%     sat.mod_bias = mean(mod_bias{1},3);sat.mod_rmse = mean(mod_rmse{1},3);
% sat.alt_mean = mean(alt_out{1},3);sat.mod_si = mod_rmse;
[X Y] = meshgrid(coord(1):res:coord(2),coord(3):res:coord(4));
sat.X = X;sat.Y = Y;sat.yearmon=yearmon
plot_alt_stats(alt_data,sat);
end
...
%     fileout = ['WW3-',alt_data(qq).sat,'-stats-',yearmon];
...
%     legend('Altimeter','Model','location','best')
%     xlabel('Longitude','fontweight','bold');
